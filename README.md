...
FinPay microservice — identity-service. Part of the [finpay-lab](https://github.com/finpay-lab) multi-repository
...
- OIDC integration with Keycloak (ADR-0006), identity aggregation.
- **AI-6 (FP-63):** `KycExtractController` + `DocumentExtractor` — KYC field extraction (vision/LLM impl), state set to REVIEW (human approval required, never auto-APPROVED).
...
Images are built, `kind load`ed as `finpaylab/identity-service:fp9`, and rolled out via the
`finpay-services` Argo CD ApplicationSet. The API entrypoint is the **gateway**
(port 8080); this service is reached internally on port 8083.
