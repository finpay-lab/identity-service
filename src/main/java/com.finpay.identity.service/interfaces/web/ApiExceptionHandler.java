...
public class ApiExceptionHandler {
    ...
    public ResponseEntity<ProblemDetail> principalNotFound(PrincipalNotFoundException ex) {
        return detail(HttpStatus.NOT_FOUND, "PRINCIPAL_NOT_FOUND", "Principal does not exist", null);
    }

    @ExceptionHandler(OidcTokenRejectedException.class)
    public ResponseEntity<ProblemDetail> tokenRejected(OidcTokenRejectedException ex) {
        return detail(HttpStatus.UNAUTHORIZED, "OIDC_TOKEN_REJECTED", ex.getMessage(), null);
    }

    @ExceptionHandler(IllegalArgumentException.class)
    public ResponseEntity<ProblemDetail> illegalArgument(IllegalArgumentException ex) {
        // Domain/boundary validation failure (e.g. unknown role name).
